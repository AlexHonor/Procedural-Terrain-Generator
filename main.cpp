...
static const GLsizei WIDTH = 2000, HEIGHT = 2000; //размеры окна
static int filling = 0;
static bool keys[1024]; //массив состояний кнопок - нажата/не нажата
static GLfloat lastX = 1000, lastY = 1000; //исходное положение мыши
...
static int createTriStrip(int rows, int cols, float size, GLuint &vao)
{
  ...
  heightmap = DiamondSquare(vertices_vec, cols, rows, size);
...
int main(int argc, char** argv)
{
  ...
  //Reset any OpenGL errors which could be present for some reason
	...
	std::unordered_map<GLenum, std::string> shaders_normals;
	shaders_normals[GL_VERTEX_SHADER] = "vertex_normals.glsl";
	shaders_normals[GL_FRAGMENT_SHADER] = "fragment_normals.glsl";
	ShaderProgram program_normals(shaders_normals); GL_CHECK_ERRORS;

	//Создаем и загружаем геометрию поверхности
	GLuint vaoTriStrip;
	size_t num = pow(2, 8) + 1;
	int triStripIndices = createTriStrip(num, num, 40, vaoTriStrip);
	
	GLuint grass = LoadTexture("tex/grass.jpg");
	BindToSlot(grass, 1);

	//GLuint snow = LoadTexture("tex/snow.jpg");
	//BindToSlot(snow, 2);

	GLuint rock = LoadTexture("tex/rock.jpg");
	BindToSlot(rock, 3);

	GLuint sand = LoadTexture("tex/sand.png");
	BindToSlot(sand, 4);
	/*
	GLuint bottom = LoadTexture("tex/bottom.jpg");
	BindToSlot(bottom, 5);
	*/

	BindToSlot(heightmap, 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glClearColor(0.2f, 0.2f, 0.2f, 1.0f); GL_CHECK_ERRORS;
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); GL_CHECK_ERRORS;

		if (normals) {
			program_normals.StartUseShader(); GL_CHECK_ERRORS;
		} else {
    ...
    //загружаем uniform-переменные в шейдерную программу (одинаковые для всех параллельно запускаемых копий шейдера)
		if (normals) {
			program_normals.SetUniform("view", view);       GL_CHECK_ERRORS;
			program_normals.SetUniform("projection", projection); GL_CHECK_ERRORS;
			program_normals.SetUniform("model", model); GL_CHECK_ERRORS;
			program_normals.SetUniform("heightmap", 0); GL_CHECK_ERRORS;
			program_normals.SetUniform("coeff", 15.0f); GL_CHECK_ERRORS;
		} else {
			...
			program.SetUniform("grass", 1); GL_CHECK_ERRORS;
			program.SetUniform("rock", 3); GL_CHECK_ERRORS;
			program.SetUniform("sand", 4); GL_CHECK_ERRORS;
			program.SetUniform("heightmap", 0); GL_CHECK_ERRORS;
			program.SetUniform("coeff", 15.0f); GL_CHECK_ERRORS;
		}
		//рисуем плоскость
